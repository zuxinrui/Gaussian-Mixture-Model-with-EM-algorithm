...
colors = ['red', 'turquoise', 'orange', 'blue']


def data2d(sigma1, sigma2, sigma3, sigma4, N, mu1, mu2, mu3, mu4, alpha):
    global X
    X = np.zeros((N, 2))
    X = np.matrix(X)
    global mu
...
def fixgmm():
    gmm = mixture.GaussianMixture(n_components=4, covariance_type='full', max_iter=40, random_state=0)
    gmm.fit(X)
...
if __name__ == '__main__':
    ...
    data2d(s1, s2, s3, s4, N, u1, u2, u3, u4, alpha)
